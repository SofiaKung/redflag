...
const QrScanner: React.FC<QrScannerProps> = ({ onScan, onClose }) => {
  ...
  const handleError = (err: any) => {
    ...
    if (err?.name === 'NotAllowedError' || err?.name === 'PermissionDeniedError') {
      setStatus('DENIED');
      setErrorMessage("Camera access was denied. Please allow camera permissions in your browser settings to use the scanner.");
    } else if (err?.name === 'NotFoundError' || err?.name === 'DevicesNotFoundError' || err?.message?.includes('device not found')) {
      setStatus('ERROR');
      setErrorMessage("No compatible camera found. If you are on a desktop, ensure your webcam is connected and not in use by another app.");
    } else {
      setStatus('ERROR');
      setErrorMessage(err?.message || "An unexpected hardware error occurred while initializing the camera.");
    }
  };
        ...
        <AnimatePresence mode="wait">
          {status === 'START' && (
            ...
            >
              ...
              <h3 className="text-white font-black text-2xl mb-3 uppercase tracking-tight">Camera Required</h3>
              <p className="text-white/50 text-sm mb-10 leading-relaxed font-medium">
                Activate the sensor to identify and intercept fraudulent QR signatures.
              </p>
              <button 
          ...
          {status === 'DENIED' || status === 'ERROR' ? (
            ...
            >
              ...
              <h3 className="text-white font-black text-xl mb-3 uppercase tracking-tight">
                {status === 'DENIED' ? 'Access Blocked' : 'Device Error'}
              </h3>
              ...
              <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center p-6">
                <div className="absolute top-32 flex flex-col items-center">
                  <div className="px-4 py-1.5 bg-red-600/20 border border-red-600/30 rounded-full flex items-center gap-3 mb-2">
                     ...
                     <span className="text-[10px] font-mono font-bold text-red-500 uppercase tracking-widest">Scanning_Target</span>
                  </div>
                </div>
      ...
      <div className="p-12 text-center bg-gradient-to-t from-black to-transparent z-20">
        <div className="max-w-xs mx-auto">
          <p className="text-white/60 text-[11px] leading-relaxed font-medium">
            Align the target within the secure perimeter. 
            <br/><span className="text-red-500 font-bold uppercase">RedFlag</span> will automatically capture the signature.
          </p>
        </div>
      </div>
    </div>
